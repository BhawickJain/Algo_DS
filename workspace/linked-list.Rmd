...
`[ ]` Create a `.exists` method that checks if a node contains a specific `data` value.  
`[ ]` Create a `.findi` method that returns the index of the node that contains the specified value in its `data` attribute.  
<!-- #endregion -->

`[x]` Create a `.iget` method that return the node object that sits at a given index.  
...
# <ipython-input-38-b2c44eefa802> in <module>
# ----> 1 L1.iget(13)

# <ipython-input-28-8b21c9fa865a> in iget(self, index)
#      87             count += 1
#      88 
# ---> 89         raise IndexError(f"Index out of Range\n{self}")

# IndexError: Index out of Range
# <Linked List> 	<Node data: 100> 
# count: 13
```
